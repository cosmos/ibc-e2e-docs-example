...
Both light clients are initialized with the attestor's Ethereum address. The script first ensures the keystore exists — generating a new one if not — then reads the address from it via `docker run`:
...
With both client IDs now known, the script registers the Cosmos-side counterparty — the on-chain record that maps the Cosmos attestation client to its EVM peer:

```bash
sandboxd tx ibc client add-counterparty $COSMOS_CLIENT_ID $EVM_CLIENT_ID ""
```

This call is idempotent: if the counterparty is already registered with the correct EVM client ID, the script skips it.

<Note>
  The EVM client registers its counterparty at `addClient` time (the Cosmos client ID is passed in the `CounterpartyInfo`). The Cosmos client registers its counterparty here, immediately after the EVM client ID is known.
</Note>
